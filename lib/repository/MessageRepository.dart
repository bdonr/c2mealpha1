...
class MessageRepository {
  ...
  addComment(DocumentReference ref, String text, File? file) async {
    ref.collection("comments").add({
      "user": repository.loggedIn!.ref,
      "text": text,
      "parentref": ref
    }).then((value) {
      value.update({"ref": value});
      if (file != null) {
        imageRepository.uploadImage(file, value, false);
      }
    });
  }
  ...
  Stream<Comment> findComment(DocumentReference id) {
    return id.snapshots().map((e) => Comment(e.get("text"),
        repository.findProfile(e["user"]), e["ref"], e["parentref"]));
  }

  Stream<List<Comment>> findCommentsOfRef(DocumentReference ref) {
    return ref.collection("comments").snapshots().map((event) {
      return event.docs
          .map((e) => Comment(e.get("text"), repository.findProfile(e["user"]),
              e["ref"], e["parentref"]))
          .toList();
    });
  }
  ...
  Stream<List<Story>> findStoriesOfUser(DocumentReference ref) {
    return ref.collection("stories").snapshots().map((e) => e.docs
        .map((f) => Story(f.id, f.get("titel"), f.get("text"),
            f.get("likeCount") ?? 0, f.reference))
        .toList());
  }
